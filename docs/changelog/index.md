...
- **Testes da API:** suíte de formulários em `apps/backend/tests/forms.test.ts` (total 7 testes passando).
...
- **API** (`apps/backend`): scaffold Express 5 + TypeScript em camadas (config, domain, application, interfaces), DI com tsyringe, log com pino, validação de env com Zod, endpoint `GET /health`, Docker Compose para Postgres local.
...
- **`npm run dev` (raiz):** agora sobe web (`:3000`) e API (`:3001`) em paralelo via `concurrently` (devDependency da raiz). Novos scripts `dev:web`/`dev:backend`.
